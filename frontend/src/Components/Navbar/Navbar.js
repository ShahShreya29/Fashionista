...
const Navbar = () => {
  ...
  const [searchOpen, setSearchOpen] = useState(false); // State to control search input visibility
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { user, isAuthenticated } = useSelector(state => state.auth);
  const cartCount = useSelector(state => state.cart.cartCount);
  ...
  const handleSearchSubmit = (event) => {
    ...
    if (searchQuery.trim() !== '') {
      // Navigate or fetch products based on searchQuery
      // Example: navigate(`/products/search/${searchQuery}`);
      console.log('Search for:', searchQuery);
      setSearchQuery('');
      toggleSearch(); // Close search input after submission
    }
  };
                  ...
                  <div className="relative flex items-center ml-auto">
                    <form onSubmit={handleSearchSubmit} className="flex items-center">
                      <InputBase
                        ...
                        className="bg-gray-100 rounded-lg px-3 py-1 focus:outline-none focus:ring-2 focus:ring-gray-300"
                      />
                      <IconButton type="submit" color="secondary">
                        <SearchIcon />
                      </IconButton>
                      <IconButton color="secondary" onClick={toggleSearch}>
                        <CloseIcon className="h-6 w-6 ml-2" aria-hidden="true" />
                      </IconButton>
                    </form>
                  </div>
                ) : (
